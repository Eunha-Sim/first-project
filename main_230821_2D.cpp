...
{
    SXVector rhs = {states(4)*cos(states(2)),
                    states(4)*sin(states(2)),
                    states(4)*tan(states(3))/WHEEL_BASE,
                    controls(1),
                    controls(0)};
    return vertcat(rhs);
}
...
pair<double, double> PurePursuit(double look_ahead_dist, double constant_velo_) { 
    double LookAheadPtX, LookAheadPtY, LookAheadPtth;
    ...
    Vector3d closest_pt = m_ref_path[m_carIdx]; 
    double closest_dist = 0.0; 

    double totalDist = 0.0;
    int lookAhead_Idx = m_carIdx; 
    cout<<"m_carIdx: " << m_carIdx << endl;


    while (totalDist < look_ahead_dist && lookAhead_Idx < m_ref_path.size() - 1) 
        {
            totalDist = DISTANCE(m_ref_path[lookAhead_Idx][0], m_ref_path[lookAhead_Idx][1], m_car.x, m_car.y); 
            lookAhead_Idx++; 
        }
        LookAheadPtX = m_ref_path[lookAhead_Idx][0];
        LookAheadPtY = m_ref_path[lookAhead_Idx][1];
        double dx = m_ref_path[lookAhead_Idx+1][0] - m_ref_path[lookAhead_Idx][0];
        double dy = m_ref_path[lookAhead_Idx+1][1] - m_ref_path[lookAhead_Idx][1];
        LookAheadPtth = atan2(dy, dx)*_RAD2DEG; 





    double actual_dist = DISTANCE(m_ref_path[m_carIdx][0], m_ref_path[m_carIdx][1], m_car.x, m_car.y);

    double rel_lookahead_x, rel_lookahead_y, rel_lookahead_th;
    GetRelativePosition(LookAheadPtX, LookAheadPtY, LookAheadPtth, m_car.x, m_car.y, m_car.th, rel_lookahead_x, rel_lookahead_y, rel_lookahead_th);

    double alpha = atan2( rel_lookahead_y,rel_lookahead_x); // not spin.. now it's spin!! but spin too much..
    //  std::cout<< "alpha: "<<alpha*_RAD2DEG << std::endl;


    double L = WHEEL_BASE;
    steerAngle = - atan2( 2.0 * L * sin(alpha) ,look_ahead_dist ) * SteerRatio *_RAD2DEG ;
    velo_result = constant_velo_;


    cout << "steerAngle[degree]: " << steerAngle << endl;


    if(abs(steerAngle) > LimitSteering)
        steerAngle = SIGN(steerAngle) * LimitSteering;
    
    cout << "steerAngle[degree](after limit): " << steerAngle << endl;
    velo_result = constant_velo_;
...
pair<double, double> Stanley(double gainK, double constant_velo_, double cross_track_err) {
    double ClosestFrontPtX, ClosestFrontPtY, ClosestFrontPtth;
    double steerAngle{0.0}; // [deg]
    double velo_result{0.0}; // [km/h]    
    ...
    int cross_track_err_Idx = m_carIdx;
    double L = WHEEL_BASE;

    while (actualDist < cross_track_err && cross_track_err_Idx < m_ref_path.size() - 1)
    {   
        //actualDist = DISTANCE(m_ref_path[cross_track_err_Idx][0], m_ref_path[cross_track_err_Idx][1], m_car.x, m_car.y  );

        actualDist = DISTANCE(m_ref_path[cross_track_err_Idx][0], m_ref_path[cross_track_err_Idx][1], m_car.x + (L*cos(m_car.th)), m_car.y + (L*sin(m_car.th)) );
        cross_track_err_Idx++;
    }
    ClosestFrontPtX = m_ref_path[cross_track_err_Idx][0];
    ClosestFrontPtY = m_ref_path[cross_track_err_Idx][1];
    double dx = m_ref_path[cross_track_err_Idx+1][0] - m_ref_path[cross_track_err_Idx][0];
    double dy = m_ref_path[cross_track_err_Idx+1][1] - m_ref_path[cross_track_err_Idx][1];
    ClosestFrontPtth = atan2(dy, dx)*_RAD2DEG; 

   
    //GetRelativePosition(m_car.x, m_car.y, m_car.th,ClosestFrontPtX, ClosestFrontPtY, ClosestFrontPtth, rel_cross_track_x, rel_cross_track_y, rel_cross_track_th);
    std::cout << "ClosestFrontPtth " << ClosestFrontPtth << std::endl; // heading error is okay 
    std::cout << "m_car.th " << m_car.th *_RAD2DEG << std::endl; // heading error is okay 


    double heading_err = (m_car.th*_RAD2DEG - ClosestFrontPtth); //this is deg

    steerAngle = (heading_err + (atan2(gainK * cross_track_err, constant_velo_)*_RAD2DEG)  ) * SteerRatio; 
    //std::cout << "atan2 " << atan2(gainK * cross_track_err, constant_velo_)  << std::endl; // heading error is okay 
...
pair<double, double> Kanayama(double K_y, double K_th, double ref_velo) {
    double ClosestPtX, ClosestPtY, ClosestPtth;
    double RefPtX, RefPtY ,RefPtth;
    double steerAngle{0.0}; // [deg]
    double velo_result{0.0}; // [km/h]  
    double FF_gain = 1.0; // Adjust this gain for setting the feed-forward term (related to the problem #1).


    double L = WHEEL_BASE;
    int kanayama_ref_idx = m_carIdx;
    double distance;
    ...
    // double dx = m_ref_path[m_carIdx+1][0] - m_ref_path[m_carIdx][0];
    // double dy = m_ref_path[m_carIdx+1][1] - m_ref_path[m_carIdx][1];
    // ClosestPtth = atan2(dy, dx)*_RAD2DEG; 


    cout << "kanayama_ref_idx: " << kanayama_ref_idx << endl;

    while (distance < 1 && kanayama_ref_idx < m_ref_path.size() - 1)
    {   
        distance = DISTANCE(m_ref_path[kanayama_ref_idx][0], m_ref_path[kanayama_ref_idx][1], m_car.x, m_car.y);
        // m_car => m 로 받아오고 m_ref_path는 m 단위이다 ref_path 간격이 인덱스당 0.01m이다
    ...
    RefPtX = m_ref_path[kanayama_ref_idx][0];
    RefPtY = m_ref_path[kanayama_ref_idx][1];
    double ref_dx = m_ref_path[kanayama_ref_idx+1][0] - m_ref_path[kanayama_ref_idx][0];
    double ref_dy = m_ref_path[kanayama_ref_idx+1][1] - m_ref_path[kanayama_ref_idx][1];
    RefPtth = atan2(ref_dy, ref_dx); //rad 

    //std::cout << "RefPtX " << RefPtX << std::endl; // heading error is okay 

    double heading_err = (m_car.th) - RefPtth; //rad

    //std::cout << "heading_err" << heading_err << std::endl; // heading error is okay 
    ...
    Jacobian << std::cos(m_car.th), std::sin(m_car.th), 0,
        -std::sin(m_car.th), std::cos(m_car.th), 0,
        0, 0, 1;
    //std::cout << "Jacobian: " << Jacobian << std::endl;

    Eigen::Vector3d ref_pose(RefPtX, RefPtY, RefPtth);
    Eigen::Vector3d curr_pose(m_car.x, m_car.y, m_car.th);
    Eigen::Vector3d err_pose = Jacobian * (ref_pose - curr_pose);
    //std::cout << "ref_pose - curr_pose: " << ref_pose - curr_pose << std::endl;  


    //std::cout << "err_pose[0]: " << err_pose[0] << std::endl;
    //std::cout << "err_pose[1]: " << err_pose[1] << std::endl;
    //std::cout << "err_pose: " << err_pose << std::endl;  // 이 error_pose 각도 값이 그냥 reference의 글로벌 값으로 나오는 거 같아

    double curvature = CalculateCurvature(kanayama_ref_idx);

    FF_term = ref_velo * KMpH2MpS * curvature;
    //std::cout << "FF_term: " << FF_term << std::endl;
    //std::cout << "curvature: " << curvature << std::endl;

    FB_term = ref_velo * KMpH2MpS * (K_y * err_pose[1] + K_th * sin(err_pose[2])) ;
    //std::cout << "FB_term: " << FB_term << std::endl;
...
// Lateral Control
    ...
    std::cout << "steerAngle[degree]: " << steerAngle << std::endl;   // problem: 코너에서 각도를 너무 많이 틀어

    if(abs(steerAngle) > LimitSteering)
        steerAngle = SIGN(steerAngle) * LimitSteering;
        std::cout << "steerAngle[degree](after limit): " << steerAngle << std::endl;
...
// Longitudinal Control
    ...
    //
   std::cout << std::endl;
    // std::cout << "m_car.x: " << m_car.x << std::endl;
    // std::cout << "sizeof(m_car.x): " << sizeof(m_car.x) << std::endl; //이게 현재 car의 위치 같은데 이게 어떻게 들어가는지 보자. 
    // std::cout << "sizeof(m_car.y): " << sizeof(m_car.y) << std::endl;
    // std::cout << "sizeof(m_car.th): " << sizeof(m_car.th) << std::endl;
    // std::cout << "sizeof(m_car.vel): " << sizeof(m_car.vel) << std::endl;
    // std::cout << "sizeof(m_car): " << sizeof(m_car) << std::endl;
...
double T = 1;
int N = 10;
double v_max = 5/3.6 , v_min = 0.0;     // LimitVelPerSec *dt /3.6
double phi_dot_max = LimitDegPerSec*_DEG2RAD /SteerRatio*dt , phi_dot_min = -phi_dot_max;
double acc_max = LimitVelPerSec *dt/3.6, acc_min = -acc_max;

double n_states =5, n_controls =2;
double velo_result{0.0}; // [km/h]  
double RefPtX, RefPtY ,RefPtth;
double steerAngle{0.0}; // [deg]
double duration;
...
SX Q = SX::diag({20.0, 20.0, 5, 0.0, 0.0});
SX R = SX::diag({0.0, 0.0});

SXVector OPT_variables;
SX st = X(Slice(), 0);

SX con, A, B, st_next, st_next_euler, push;
SXDict nlp_prob;
Dict opts;


int N_control_points = N + 1; // 10+1
int num_decision_variables = 5 * N_control_points + 2 * N;

SX args_lbg(5*(N_control_points),1), args_ubg(5*(N_control_points),1);
...
pair<double,double> MPC() { //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // cout << "pi_dot limit[rad]: " <<  LimitDegPerSec*_DEG2RAD /SteerRatio*dt << endl;
...
//////////////////// while문 시작 ///////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // cout<<"m_carIdx: " << m_carIdx << endl;       
            ...
            double phi_ref = 0.0; double vel_ref = 0.0;
            double u_ref = 0.0, phi_dot_ref = 0.0;

            P(7 * k + 5) = x_ref; P(7 * k + 6) = y_ref; P(7 * k + 7) = theta_ref; P(7 * k + 8) = phi_ref;  P(7 * k + 9) = vel_ref;//여기에 이상한 값이 들어가나?
            P(7 * k + 10) = u_ref; P(7 * k + 11) = phi_dot_ref;
         }

        //  cout << "P: " << P << endl;
        // std::cout << "x_ref[m]: " <<  m_ref_path[ref_index_][0] << std::endl;
        // std::cout << "y_ref[m]: " <<  m_ref_path[ref_index_][1] << std::endl;
        // std::cout << "theta_ref[rad]: " <<  m_ref_path_theta[ref_index] << std::endl;

        cout << "ref_index(빨간점 위치): " << ref_index_ << endl;


        SX args_x0 = SX::vertcat({reshape(X0.T(), 5 * N_control_points, 1),  reshape(u0.T(), 2 * N, 1) }); //여기 다시해야겠다.
        ...
        DM u = reshape(sol["x"](Slice(5 * (N + 1), sol["x"].size1()) ).T(), 2,N).T(); //이거 크기 10*2 이다. 



        // ////////////////////////path 계산한 부분 visualizing하는 부분.///////////////////////////////////////////
        ...
        for(int i=0; i < N+1; i++)
        {
            ...
            gui_path.poses[i].pose.position.x = double(sol["x"](5 * i));
            gui_path.poses[i].pose.position.y = double(sol["x"](5 * i + 1));
            gui_path.poses[i].pose.position.z = 0.0;
            // tf2::Quaternion q;
            // q.setRPY( 0, 0, PathPlan(i,2) );
...
/////////////////////// shift 함수 정의하기//////////////////////////////////////////////
 ...
 //////////////////////////////////////////////////////////////////////////////////////////
        ...
        velo_result = double(x0(4))*3.6;
        cout << "x0(4)[m/s]: " << x0(4) << endl;
        ...
        // std::cout << "RefPtX: " << RefPtX << std::endl;
        // std::cout << "RefPtY: " << RefPtY << std::endl;

        // std::cout << "m_car.x: " << m_car.x << std::endl;
        // std::cout << "m_car.y: " << m_car.y << std::endl;
        // std::cout << "m_car.th: " << m_car.th << std::endl; 
        // cout << "hz: " << CLOCKS_PER_SEC/duration << endl;
        

        // cout << "theta[degree],x0(2): " << double(x0(2))*_RAD2DEG << std::endl; 

        X0 = reshape(sol["x"](Slice(0,5*(N+1))).T() ,5 ,N+1).T();
        ...
        return std::make_pair(steerAngle, velo_result); // steerAngle 과 velo_result를 보내준다.
...
///////////////////////MPC 추가한 부분 /////////////////////////////////////
    ...
    x0 = {m_ref_path[0][0], m_ref_path[0][1], m_ref_path_theta[0], 0.0 , 1.0/3.6}; // 이게 while문 앞에 있으면 x0를 기억해서 못 쓰게 돼. 
    // xs = {m_ref_path[m_ref_path.size()-2][0], m_ref_path[m_ref_path.size()-2][1], m_ref_path_theta[m_ref_path.size()-2], 0.0}; 
    xs = {m_ref_path[0][0], m_ref_path[0][1], m_ref_path_theta[0], 0.0 ,0.0};
    t(0) = t0; 
    ...
    if (!m_finish_flag && m_ref_path.size() > 1) {
        ...
        switch(LOCAL_TRACKER) {
            ...
            case 2: {// Stanley
                ...
                _control = Stanley(1.0, 5.0, min_dist);                
                break;
            }

            case 3: {// Kanayama
                CalculateClosestPt(pWIdx, m_car.x, m_car.y); // Computing the closest point from the rear axle
                _control = Kanayama(1.0, 3.0, 3.0);
                break;
            }
...
std::cout <<"m_ref_path.size()여기가 되나 2 ??: " << m_ref_path.size() << std::endl;
...
void MPC_init() {  
    ...
    for (int k = 0; k < N; ++k) {    
        ...
        A = P(Slice(7*k+5,7*k+10)); 
        B = P(Slice(7*k+10,7*k+12));
        error_pose = st - A;

        error_pose(2) = atan2(sin(error_pose(2)), cos(error_pose(2)));
        obj = obj + ( mtimes(mtimes((error_pose).T(),Q) , (error_pose))) +  (mtimes(mtimes((con-B).T() , R) ,(con-B) ) + mtimes(mtimes((con-B).T() , R) ,(con-B) ) ); 
    ...
    OPT_variables.push_back(reshape(X, 5 * (N+1), 1));
    ...
    // SX args_lbx(num_decision_variables,1), args_ubx(num_decision_variables,1); 
-
    args_lbg(Slice(0,5*(N + 1)),0) = 0.0;    //Equality constraints.
    args_ubg(Slice(0,5*(N + 1)),0) = 0.0;

     //                    행         열
    args_lbx(Slice(0, 5*(N + 1), 5) ,0) = -inf; // state x lower bound  
    args_ubx(Slice(0, 5*(N + 1), 5) ,0) = inf; // state x upper bound
    args_lbx(Slice(1, 5*(N + 1), 5) ,0) = -inf; // state y lower bound
    args_ubx(Slice(1, 5*(N + 1), 5) ,0) = inf; // state y upper bound
    args_lbx(Slice(2, 5*(N + 1), 5) ,0) = -inf; // state theta lower bound
    args_ubx(Slice(2, 5*(N + 1), 5) ,0) = inf; // state theta upper bound
    ...
    args_lbx(Slice(3, 5*(N + 1), 5) ,0) = -35*_DEG2RAD; // state phi lower bound
    args_ubx(Slice(3, 5*(N + 1), 5) ,0) = 35*_DEG2RAD; // state phi upper bound
    args_lbx(Slice(4, 5*(N + 1), 5) ,0) = v_min; // state velo lower bound
    args_ubx(Slice(4, 5*(N + 1), 5) ,0) = v_max; // state velo upper bound

    args_lbx(Slice(5 * (N+1), 5*(N+1)+2*N, 2) ,0) = acc_min; // acc lower bound
    args_ubx(Slice(5 * (N+1), 5*(N+1)+2*N, 2) ,0) = acc_max; // acc upper bound
    args_lbx(Slice(5 * (N+1)+1, 5*(N+1)+2*N, 2) ,0) = phi_dot_min; // phi_dot lower bound
    args_ubx(Slice(5 * (N+1)+1, 5*(N+1)+2*N, 2) ,0) = phi_dot_max; // phi_dot upper bound
