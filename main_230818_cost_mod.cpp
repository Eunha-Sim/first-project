...
//N: prediction horizon
    ...
    double v_max = 5/3.5/60.0 , v_min = 1.0/3.5/60.0;     
    // double phi_dot_max = LimitDegPerSec*_DEG2RAD/60.0*0.05, phi_dot_min = -phi_dot_max;
    double phi_dot_max = LimitDegPerSec*_DEG2RAD/60.0*0.08, phi_dot_min = -phi_dot_max;
    ...
    SX Q = SX::diag({3.8, 3.8, 25.5, 0.0001});
    SX R = SX::diag({0.0, 0.0});
    SX R_u = SX::diag({200.0, 100.0});
...
pair<double,double> MPC() { //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //std::cout <<"m_ref_path.size(): " << m_ref_path.size() << std::endl;


    // finish = clock();
    // duration = (finish - start);
    // std::cout <<"duration(ref_path 만드는 시간): " << duration << std::endl;
...
/////////////////////// shift 함수 정의하기//////////////////////////////////////////////
        ...
        // SX st_sh = x0; 
        // SX con_sh = u(0, Slice()).T();
        // st_sh = st_sh + (T*rhs(st_sh, con_sh));
        // // st_sh = st_sh + (T*rhs(st_sh, con_sh));
        // cout << "con_sh: " <<con_sh <<endl;
        // cout << "st_sh: " <<st_sh <<endl;
        // x0 = st_sh;
        // t0 = t0 + T;
...
//////////////////////////////////////////////////////////////////////////////////////////
        ...
        // return std::make_pair(steerAngle, velo_result); // 얘내를 어디에 둬야할까. 여기가 carla랑 통신하는 부분인 것 같다. 
        return std::make_pair(steerAngle, velo_result);
...
void VehicleControl(double steer, double velocity) {
    ...
    // For CARLA   //CARLA�� �����ϴ� �κ�. �޼����� �����ϴ� �κ��ΰ��� (push)
...
void MPC_init() {  
    ...
    for (int k = 0; k < N; ++k) {    
        ...
        if (k!=0){
            // obj = obj+ (u(k)-u(k-1 ))^T * R_u * (u(k,0)-u(k-1,0));
